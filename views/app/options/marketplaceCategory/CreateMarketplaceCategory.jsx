...
const CreateMarketplaceCategory = ({ onCancel, data, ...props }) => {
  const { status } = useSelector(
    ({ marketplaceCategory }) =>
      marketplaceCategory.createMarketplaceCategoryAction
  );
  const { status: updateStatus } = useSelector(
    ({ marketplaceCategory }) =>
      marketplaceCategory.updateMarketplaceCategoryAction
  );
  ...
  const onFinish = ({ ...values }) => {
    ...
    if (!isEmpty(data)) {
      dispatch(
        Actions.updateMarketplaceCategory({
          ...payload,
          translate: {
      ...
      dispatch(
        Actions.createMarketplaceCategory({
          ...payload,
          translate: {
            data: [{ key: values.name }, { key: values.description }],
          },
  ...
  return (
    <Modal
      ...
      width={710}
      title={
        isEmpty(data) ? 'New Marketplace Category' : 'Edit Marketplace Category'
      }
    ...
    >
      ...
      >
        ...
        <Form.Item required label={<h6 className="input-label">Cover</h6>}>
...
export default CreateMarketplaceCategory;
