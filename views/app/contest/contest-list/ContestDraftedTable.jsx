...
import { getDraftedContests } from '../../../../redux/actions';
import UpdateContest from './ContestCreateEdit';
import DeleteContest from './DeleteContest';

const ContestDraftedTable = () => {
  const { data: draftedContests, loading } = useSelector(
    ({ contest }) => contest.draftedContests
  );
  ...
  useEffect(() => {
    if (status === 'submitted') dispatch(getDraftedContests());
  }, [dispatch, status]);

  useEffect(() => {
    dispatch(getDraftedContests());
  }, [dispatch]);
...
export default ContestDraftedTable;
