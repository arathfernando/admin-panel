...
import { getPendingContestss } from '../../../../redux/actions';
import UpdateContest from './ContestCreateEdit';
import DeleteContest from './DeleteContest';

const ContestPendingTable = () => {
  const { data: pendingContests, loading } = useSelector(
    ({ contest }) => contest.pendingContests
  );
  ...
  useEffect(() => {
    if (status === 'submitted') dispatch(getPendingContestss());
  }, [dispatch, status]);

  useEffect(() => {
    dispatch(getPendingContestss());
  }, [dispatch]);
...
export default ContestPendingTable;
