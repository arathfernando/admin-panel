...
import { getCompletedContests } from '../../../../redux/actions';
...
const ContestCompletedTable = () => {
  const { data: completedContests, loading } = useSelector(
    ({ contest }) => contest.completedContests
  );
  ...
  useEffect(() => {
    if (status === 'submitted') dispatch(getCompletedContests());
  }, [dispatch, status]);

  useEffect(() => {
    dispatch(getCompletedContests());
  }, [dispatch]);
...
export default ContestCompletedTable;
